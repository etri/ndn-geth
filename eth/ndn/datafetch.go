...
func (m ObjManager) GetSegment(query ndnapp.Query, segId uint16) (msg ndnapp.ResponseWithDataSegment, err error) {
	var segment []byte //segment
	var num uint16 //number of segments
	ethquery,ok := query.(ndnapp.HasId)
	if !ok {
		err = ErrUnexpected
		return
	}
	...
	if segId > 0 {
		...
		if err == nil {
			...
			msg = &EthMsg{
				...
				Content:	ndnapp.NewSimpleSegment(segment, segId, num),
			}
		}
		return
	}
...
type EthObjectFetcher struct {
	...
	pfetcher	ndnapp.AsyncAppConsumer
	appname		ndn.Name
}

func (f *EthObjectFetcher) fetch(query ndnapp.Query, route *PeerRouteInfo, abort chan bool) ([]byte, error) {
	route.appname = f.appname
	return f.fetcher.FetchObject(query, route, abort)
}

func (f *EthObjectFetcher) pfetch(query ndnapp.Query, route *PeerRouteInfo, numsegments uint16, abort chan bool) ([]byte, error) {
	route.appname = f.appname
...
type objFetchingJob struct {
	...
	route		*PeerRouteInfo
	query		ndnapp.Query	 //object query
	numsegments	uint16
...
func newObjFetchingJob(fetcher *EthObjectFetcher, query ndnapp.Query, numsegments uint16, route *PeerRouteInfo, fn OnObjFetchedHandler) *objFetchingJob {
...
func (j *objFetchingJob) queryId() string {
	if q, ok := j.query.(ndnapp.HasId); ok {
		return q.GetId()
	}
	return "" 
}
