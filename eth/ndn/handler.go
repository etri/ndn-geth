...
type Controller struct {
	...
	fpool			utils.WorkerPool //worker pool for handling block/transaction fetching jobs
	hpool			utils.WorkerPool //worker pool for handling incomming messages
	opool			utils.WorkerPool //worker pool for sending outgoing messages
	...
	ethconsumer			ndnapp.AppConsumer //for sending sync NDN Interest
	ethasyncconsumer	ndnapp.AsyncAppConsumer //for sending async NDN Interest
...
func (c *Controller) ndnsender() ndnapp.Sender {
...
func (c *Controller) Start(server *kad.Server, maxPeers int) {
	...
	c.ethconsumer = ndnapp.NewAppConsumer(ethMsgResponseDecoder{}, c.ndnsender())
	c.ethasyncconsumer = c.ethconsumer.Attach(c.opool)
...
func (c *Controller) registerProducers() {
	prefix := ndnapp.BuildName(c.server.Config.HostName, c.server.Config.AppName)

	prv_prefix := ndnapp.BuildName(prefix, []ndn.NameComponent{EthNdnName})
	pub_prefix := ndnapp.BuildName(c.server.Config.AppName, []ndn.NameComponent{EthNdnName})
	rpc_prefix := ndnapp.BuildName(prefix, []ndn.NameComponent{EthRpcNdnName})

	mixer := c.server.Transport()

	objmanager := newObjManager(c) 

	ethfn := func(req *ndnapp.ProducerRequest) {
	...
	mixer.Register(ndnapp.NewAppProducer(prv_prefix, mixer.Sender(), ethdecoder, ethfn, objmanager, c.hpool))

	mixer.Register(ndnapp.NewAppProducer(pub_prefix, mixer.Sender(), ethdecoder, ethfn , objmanager, c.hpool))

	c.rpchandler = newRpcHandler(c)
	//rpcserver := ndnapp.NewObjServer(c.rpchandler)
	rpcdecoder := rpcRequestDecoder{}
	mixer.Register(ndnapp.NewAppProducer(rpc_prefix, mixer.Sender(), rpcdecoder, nil, c.rpchandler, nil))
}
...
func (c *Controller) asendEthInterest(p *peer, msg *EthMsg, usehint bool, fn func(ndnapp.Request, interface{}, ndnapp.Response, error)) error {
	routeinfo := &PeerRouteInfo {
		peername:	p.prefix,
		usehint:	usehint,
		appname:	c.appname,	
...
func (c *Controller) receiveStatus(i *ndn.Interest, msg *StatusMsg, producer ndnapp.AppProducer) {
...
func (c *Controller) receiveBye(i *ndn.Interest, msg *ByeMsg, producer ndnapp.AppProducer) {
...
func (c *Controller) receiveEthMsg(i *ndn.Interest, msg *EthMsg, producer ndnapp.AppProducer) {
	if msg == nil {
		c.sendInterestAck(i, producer)
		return
	}
...
func (c *Controller) receivePushMsg(i *ndn.Interest, msg *PushMsg, producer ndnapp.AppProducer) {
	id := msg.NodeId
...
func (c *Controller) forwardEthAckMsg(p *peer, msg *EthMsg) {
	c.hpool.Do(&ackMsgHandlingJob{p: p, msg: msg, c: c})
}

//send an empty acknowledgement message
func (c *Controller) sendInterestAck(i *ndn.Interest, producer ndnapp.AppProducer) {
...
func (c *Controller) replyBye(i *ndn.Interest, producer ndnapp.AppProducer) {
