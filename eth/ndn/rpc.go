...
func (msg *RpcMsg) ExtractQuery() (query ndnapp.Query, segId uint16, err error) {
	switch msg.MsgType {
		case RMT_Command:
			segId = 0
...
func (msg *RpcMsg) WriteData(i *ndn.Interest) (d ndn.Data) {
	if objsegment, ok := msg.Content.(*ndnapp.SimpleSegment); ok {
...
func (msg *RpcMsg) ExtractSegment() (seg ndnapp.ObjSegment, err error) {
...
func (req *RpcCommand) WriteRequest(segId uint16) ndnapp.Request {
	if segId == 0 {
		return &RpcMsg {
			MsgType:	RMT_Command,
			Content:	req,
...
func (e rpcRequestDecoder) DecodeInterest(i *ndn.Interest) (msg ndnapp.Request, err error) {
...
func (e rpcResponseDecoder) DecodeData(d *ndn.Data) (msg ndnapp.Response, err error) {
	var segment ndnapp.SimpleSegment
...
func (c *Controller) receiveRpcMsg(i *ndn.Interest, msg *RpcMsg, producer ndnapp.AppProducer) {
}

type RpcClient struct {
	fetcher		ndnapp.AppConsumer
}



func NewRpcClient(sender ndnapp.Sender) *RpcClient {
	decoder := rpcResponseDecoder{}
	consumer := ndnapp.NewAppConsumer(decoder, sender)
	return &RpcClient {
		fetcher: consumer,
	}
}
...
func (rpc *RpcHandler) GetSegment(query ndnapp.Query, segId uint16) (msg ndnapp.ResponseWithDataSegment, err error) {
	...
	if err == nil {
		if response != nil { //first segment
			wire, _ := response.MarshalJSON()
				...
				msg = &RpcMsg{
					MsgType:	RMT_Segment,
					Content:	ndnapp.NewSimpleSegment(segment, 0, num),
				}
			...
			if req, ok := query.(*RpcSegmentReq); ok {
				if segment, num, err = rpc.cacher.GetSegment(req.Id, req.SegId); err == nil {
					...
					msg = &RpcMsg{
						MsgType:	RMT_Segment,
						Content:	ndnapp.NewSimpleSegment(segment, req.SegId,num),
					}
