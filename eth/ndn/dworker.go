...
type dwork struct {
	...
	bpool		utils.WorkerPool //pool for downloading block
	ppool		utils.WorkerPool //pool for downloading block parts
...
func (d *Downloader) fetchFullBlocks(w *dwork) error {
	...
	for h := range w.headerCh {
		...
		if !w.bpool.Do(newdownloadBlockJob(d, w, h, onfetched)) {
