...
func (q *EthGenericQuery) WriteRequest(segId uint16) ndnapp.Request {
	return &EthMsg{
		MsgType:	TtDataReq,
...
func (q *EthTxQuery) WriteRequest(segId uint16) ndnapp.Request {
...
func (q *EthHeadersQuery) WriteRequest(segId uint16) ndnapp.Request {
...
func (q *EthBlockQuery) WriteRequest(segId uint16) ndnapp.Request {
...
func writeRequest(q ndnapp.Query, qtype byte, segId uint16) ndnapp.Request {
	if segId == 0 { 
		return &EthMsg{
			MsgType:	TtDataReq,
			Content:	&EthDataReqMsg {
...
func (msg *EthMsg) ExtractQuery() (query ndnapp.Query,segId uint16,err error) {
...
func (msg *EthMsg) MarshalTlv() (typ uint32, value []byte, err error) {
	if msg.MsgType == TtDataSegment {
		seg, _ := msg.Content.(*ndnapp.SimpleSegment)
...
func (msg *EthMsg) UnmarshalTlv(typ uint32, value []byte) (err error) {
	...
	case TtDataReq:
		...
		msg.Content = &content
	case TtDataSegment:
		var content ndnapp.SimpleSegment
		err = rlp.DecodeBytes(value, &content)
		msg.Content = &content 
...
func (msg *EthMsg) ExtractSegment() (seg ndnapp.ObjSegment, err error) {
	if msg.MsgType != TtDataSegment {
		err = ErrUnexpected
	} else {
		seg, _ = msg.Content.(ndnapp.ObjSegment)
	}
	return
}
...
func (msg *EthMsg) WriteInterest(route interface{}) (i ndn.Interest) {
	...
	if msg.MsgType == TtDataReq {
		...
		if req.QType == QtGenericQuery {
			...
			querystr = fmt.Sprintf("/%s/%s/%d",REQSTR,req.Query.(ndnapp.HasId).GetId(),req.Segment)
			//query name is /req/queryid/segid
			queryname = ndn.ParseName(querystr)
		}
	}
	// routename is /blockchain/eth
	routename := ndnapp.BuildName(routeinfo.appname, []ndn.NameComponent{EthNdnName})
	...
	if routeinfo.usehint { //set forwardinghint
		...
		hintname := ndnapp.BuildName(routeinfo.peername, routeinfo.appname)
		delegation := ndn.MakeFHDelegation(0, hintname)

		if len(queryname) > 0 {
			iname := ndnapp.BuildName(routename, queryname)
		...
		if len(queryname) >0 {
			i = ndn.MakeInterest(ndnapp.BuildName(routeinfo.peername, routename, queryname))
		} else {
			i = ndn.MakeInterest(ndnapp.BuildName(routeinfo.peername, routename), wire)
		}
	}
	//if msg.MsgType != TtDataReq {
...
func (enc ethMsgResponseDecoder) DecodeData(d *ndn.Data) (msg ndnapp.Response, err error) {
	var ethmsg EthMsg
	if len(d.Content) > 0 {
...
func (enc ethMsgRequestDecoder) DecodeInterest(i *ndn.Interest) (msg ndnapp.Request, err error) {
	var ethmsg EthMsg
