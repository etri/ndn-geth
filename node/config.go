...
//	ndn "github.com/ethereum/go-ethereum/ndn/gondn"
...
const (
	...
	datadirBootNodes    = "bootnodes.json" // Path within the datadir to the bootnode list
)
...
func (c *Config) Bootnodes() []p2p.NodeRecord {
	var err error
	emptylist := []p2p.NodeRecord{}
	if c.DataDir == "" {
		return emptylist
	}

	bootnodesfile := c.ResolvePath(datadirBootNodes)
	type BootNode struct {
		Id string
		Prefix	string
	}
	var nodelist []BootNode	
	if err = common.LoadJSON(bootnodesfile, &nodelist); err != nil {
		log.Error(fmt.Sprintf("No bootnodes.json => no discovery"))
		return emptylist
	}
	if len(nodelist) == 0 {
		return emptylist
	}
	noderec := make([]p2p.NodeRecord, len(nodelist))
	i := 0
	for _, n := range nodelist {
		if noderec[i], err = p2p.CreateUntrustedNdnNodeRecord(n.Id, n.Prefix); err == nil {
			i++
		}
	}
	return noderec[:i]
