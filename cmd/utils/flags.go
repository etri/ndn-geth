...
var (
	...
	MaxPeersFlag = cli.IntFlag{
		...
		Value: eth.DefaultConfig.MaxPeers,
	}
	/*
	...
	*/
	NodeHostNameFlag = cli.StringFlag{
		Name:  "ndnhostname",
		Usage: "NDN node name",
	}

	NodeNDNSocketFlag = cli.StringFlag{
		Name:  "ndnsocket",
		Usage: "Unix socket for NFD",
		Value: "/run/nfd.sock",
	}
...
//set ndn node name and network name
func setNodeAddress(ctx *cli.Context, cfg *p2p.Config) {
	var (
		hostname  = ctx.GlobalString(NodeHostNameFlag.Name)
	)
	cfg.HostName = ndn.ParseName(hostname)
	cfg.AppName = ndn.ParseName("/blockchain")
}
...
func SetNodeConfig(ctx *cli.Context, cfg *node.Config) {
	...
	if ctx.GlobalIsSet(NodeNDNSocketFlag.Name) {
		cfg.NDNSocket = ctx.GlobalString(NodeNDNSocketFlag.Name)
	}
...
func SetEthConfig(ctx *cli.Context, stack *node.Node, cfg *eth.Config) {
	cfg.ExpFileName = stack.ResolvePath("traffic.txt")
	cfg.BlocksFileName = stack.ResolvePath("blocks.log")
...
func RegisterShhService(stack *node.Node, cfg *whisper.Config) {
	Fatalf("Whisper is not supported in NDN")
	/*
